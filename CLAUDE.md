...
*(Delivered in 2.2.0 — the `Str`/`Num`/`Dt` additions, the `Arr` dot-path family, and the `Id` inspection helpers that lived here are now in `CHANGELOG.md`. `Id` inspection landed on `Rand` rather than a new class.)*
