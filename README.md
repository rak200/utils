...
[![Mutation testing](https://img.shields.io/badge/Infection-min%20covered%20MSI%2080%25-brightgreen?logo=php&logoColor=white)](infection.json5.dist)
...
- **`Arr`** — array operations: emptiness, count, first/last (value and key) lookup, find/search/keyPosition, filter/map/flatMap/reduce, flatten, groupBy, partition, chunk, unique, reverse/slice/flip/combine, diff/intersect, countValues, append/prepend, immutable shift/pop (`[element, rest]`) and removeAt (pure splice), fill, sort/sortBy (optionally key-preserving)/sortKeys, zip, range, nested dot-path access (`get`/`set`/`forget`/`dot`/`undot`).
- **`Iter`** — the lazy, generator-returning twin of `Arr` for streaming and infinite sequences: sources (`range`/`repeat`/`cycle`/`iterate`/`times`), lazy transforms (`map`/`filter`/`flatMap`/`take`/`drop`/`takeWhile`/`dropWhile`/`chunk`/`flatten`/`zip`/`concat`/`keys`/`values`/`unique`/`slice`/`tap`), and consuming terminals (`first`/`last`/`find`/`nth` (+`*OrNull`), `reduce`/`count`/`contains`/`any`/`every`/`toArray`). Single-pass: a generator is consumed once.
- **`Num`** — number operations: type checks (incl. `isFinite`/`isNan`/`isInfinite`), parsing (incl. `parseNumber` returning `BcMath\Number`), `toStr` (the exact, round-tripping string form of a float) and `toBase`, clamp/inRange/`lerp`/`remap`, rounding, formatting, aggregates (`sum`/`product`/`avg`/...), sign, arithmetic (`add`/`sub`/`mul`/`div`, `pow`, `sqrt`, `floor`, `ceil`, `mod`). Aggregation and per-element methods accept and propagate `BcMath\Number` alongside `int|float`.
...
- **`Filter`** — input sanitisation and lenient coercion of untrusted values: `escapeHtml`/`unescapeHtml`, `stripTags`, character whitelists (`digits`/`alpha`/`alnum`), `squish`/`stripControl`/`ascii`, `email`/`url` (sanitise) and `isEmail` (validate), and `mixed`-to-typed coercers with a default (`toStr`/`toInt`/`toFloat`/`toBool`).
