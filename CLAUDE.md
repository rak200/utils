...
- **`Iter`** — lazy counterpart of `Arr`, operating on `array|Traversable` and returning `Generator` so transforms compose without materialising intermediate arrays (enables infinite sequences and streaming over `File::lines`). Its *sole* justification is laziness — so it reuses `Arr`'s names where the operation matches (`map`/`filter`/`first`/`last`/`chunk`/`flatten`/`zip`/`unique`/`reduce`/`count`/`slice`/`contains`/`find`, with `*OrNull` for the not-found cases), reading as the eager/lazy twin of `Arr`. Sources: `range` (unbounded), `repeat`, `cycle`, `iterate`, `times`. Lazy transforms additionally: `take`/`drop`/`takeWhile`/`dropWhile`/`flatMap`/`keys`/`values`/`concat`. Terminals materialise/consume: `toArray(bool $preserveKeys = false)` (reindexes by default — lazy transforms preserve keys, so `concat`/`zip`/repeated keys would otherwise collide), `nth`, `any`/`every`. **The one place the immutable/stateless contract bends:** a `Generator` is single-pass — passing one to two terminals leaves the second empty. Unavoidable without buffering (which would defeat laziness), so it must be documented loudly rather than worked around. Impure `each` (side-effecting `foreach`) stays out, same reasoning as the mutable natives below; use a lazy `tap` if a peek is needed. Purely additive — lands in a minor when real demand appears.
...
- **`Arr` dot-path access** — `get`/`has`/`set`/`forget` taking `'a.b.c'` nested keys, plus `dot`/`undot` to flatten ↔ expand. Very common need, pure and immutable-friendly (`set`/`forget` return a new array). Current gap: `Arr` has `pick`/`except`/`pluck`/`keyBy` but no nested-path access.
- **`Id` inspection / validation** — the missing read side of `Rand`'s ID *generation*: `isUuid`/`isUlid` (+ version/variant checks) and extracting the embedded timestamp from a UUID v7 / ULID. Open decision: live on `Rand` or split a new `Id` class (generation = entropy stays in `Rand`; validating/parsing an existing ID is a different concern). `Id` is already a sanctioned abbreviation in the naming rules.
- **`Str` additions** — `mask` (mask part of a string for PII: card numbers, emails) and `slug` (URL-safe slug). Both pure and frequently wanted.
- **`Num` additions** — `remap`/`lerp` (map a value from one numeric range to another). Pure; pairs with the existing `clamp`/`inRange`.
- **`Dt` additions** — a period iterator (yields `DateTimeImmutable` across a range; pairs with `Iter`) and predicate helpers `isWeekend`/`isPast`/`isFuture`.
