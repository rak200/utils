...
- **List-preserving generics on `Arr::map`.** `Arr::map` declares `@return array<K, TResult>`, so mapping a `list<T>` erases the list-ness PHPStan knew about, and downstream code needs a localized `@var` to restate it (first real hit: http-input's `Validator::messages()`, which maps `{field: list<InputException>}` to `{field: list<string>}`). Mapping never disturbs a list's `0..n-1` keys, so a conditional return type — `@return ($array is list<T> ? list<TResult> : array<K, TResult>)` — restores the information. PHPDoc-only, BC-safe; while at it, audit the other helpers whose output is provably a list when the input is one.
