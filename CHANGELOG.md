...
* the extensions in use are declared, and the optional one is suggested ([#85](https://github.com/rak200/utils/issues/85)) ([348b02f](https://github.com/rak200/utils/commit/348b02f671e47643f88f427223d954abf6172ab2))
...
* mark the pre-push hook executable ([#43](https://github.com/rak200/utils/issues/43)) ([eaf03ce](https://github.com/rak200/utils/commit/eaf03ce2d83956f1ac48be4dfb3ab4664eff0d47))
...
- **`Rak200\Utils\Exception` — the library-scoped catch.** 4.0.0 mapped every throw-site to the precise SPL type so callers could branch on the failure *kind*; what SPL cannot express is "a failure from *this* library" as one clause. The new namespace closes that: a `UtilsException` marker interface (`extends Throwable`) implemented by empty-bodied domain classes, each extending the SPL type its kind already used — `MalformedArgumentException` (`InvalidArgumentException` — malformed input / out-of-domain argument), `LookupException` (`OutOfBoundsException` — lookup with no result), `EmptySourceException` (`UnderflowException` — empty source), `BadCallbackException` (`UnexpectedValueException` — bad callback result), `MalformedJsonException` (the native `\JsonException`), and one two-level branch: an abstract `IOException` (`RuntimeException` — environment/native I/O failure, the grouping node future I/O domains slot under) whose sole concrete leaf today is `FilesystemException`, raised only by `File`. All 130 throw-sites now raise their domain class with messages unchanged, and **BC holds by inheritance**: every existing `catch` of an SPL type keeps matching — it now catches a subclass. `Json::encode`/`decode` needed more than a rename, since they let the native `\JsonException` escape from `JSON_THROW_ON_ERROR`: both now catch it and rethrow `MalformedJsonException` with the message, the `JSON_ERROR_*` code and the native as `getPrevious()` — nothing a caller could read is lost, and the wrap is pinned by tests asserting all three. The suite now pins the lib types, not the SPL parents: the 135 lib-facing `expectException` calls (and `RegexTest`'s catch-based sweep) reference the domain classes, so a throw-site reverted to its SPL type fails a test instead of passing by ancestry — the two deliberately-native spots stay native (`Dt::parseOrNull`'s internal `catch (Exception)`; the engine-raised drained-generator assert in `IterTest`). Reference page: `docs/exceptions.md`.
