...
Convert between a binary string and its hexadecimal form: `encode` produces lowercase, two digits per byte (never fails); `decode` is the inverse and accepts upper- and lowercase digits. `decode` throws `MalformedArgumentException` on invalid hex (odd length or a non-hex character).
