...
final class Path
{
     ...
     *
     * @throws MalformedArgumentException when $from and $to cannot be related (mismatched
     *                                    anchors or drives)
     */
    public static function relative(string $from, string $to): string
    {
        ...
        if (self::isAbsolute($fromNorm) !== self::isAbsolute($toNorm)) {
            throw new MalformedArgumentException('Cannot compute relative path between absolute and relative paths.');
        }
        ...
        if ($fromDrive !== $toDrive) {
            ...
            throw new MalformedArgumentException("Cannot compute relative path across drives {$fromLabel} and {$toLabel}.");
        }
